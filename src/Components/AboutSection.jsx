...
const AboutSection = () => {
  return (
    <div className="bg-about-image bg-cover bg-no-repeat mt-20" id="about">
      <p className="font-medium text-sm border border-gray-300 rounded-xl px-3 py-1 w-44 m-auto">
      ...
      <h2 className="bg-gradient-to-b from-[#000000] to-[#001354] text-transparent bg-clip-text font-bold font-DM flex flex-col text-3xl md:text-6xl py-10 tracking-tighter text-center">
        ...
        <span className="text-center">track progress</span>
      </h2>
      ...
      <div className="relative">
        <img src={ProductImage} className="m-auto" />
        <img
          src={Torus}
          className="absolute left-0 bottom-36 hidden sm:block"
        />
        <img
          src={Pyramid}
          className="absolute right-0 lg:top-44 md:-top-28 hidden sm:block"
        />
      </div>
      <div className="grid md:grid-cols-2 gap-y-20 mt-10 lg:grid-cols-4 px-14 gap-5 pb-32 place-items-center">
        {grid.map((item) => (
          <div key={item} className="space-y-2 w-[260px]">
            ...
            <h3 className="font-DM font-bold text-lg tracking-wide">
              {item.title}
            </h3>
            <p className="font-inter font-medium tracking-wide text-base">
              {item.text}
            </p>
            <button className="flex items-center font-medium text-base tracking-wide">
              Learn more
              <span className="px-2">
                <FaArrowRight />
              </span>
